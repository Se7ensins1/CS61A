...
(define (has-cycle s)
  (define (helper s original)
    (cond
        ((null? s) False)
        ((eq? (cdr-stream s) original) True)
        (else (helper (cdr-stream s) original))
    )
  )
  (helper s s)
)
...
(begin (define x 8649) (+ 2 3))
;expect 5

(begin (print 3) (newline) 1)
;expect 1
...
;expect (lambda (x y) (lambda (x) (+ x y)))
...
(define (root x) (expt x .5))
;expect root
(root 9)
;expect 3.0
(root 17)
;expect 4.123105625617661
...
(let ((s 'snake) (y 'butt)) (list s y))
;expect (snake butt)

(define s (let ((x 5) (y 9)) (cons x y)))
;expect s

(let ((cal 'go) (wins 'bears)) (cons cal wins))
;expect (go . bears)
